    ...
    return wrap(
      <>
        ...
        <div style={{ position:'absolute', left: 56, right: 56, top: 320, textAlign:'center' }}>
          ...
          <h1 style={{ fontFamily: fonts.display, fontWeight: 400, fontSize: 140, lineHeight: 1.0, letterSpacing: '-0.025em', margin: 0, fontStyle:'italic', textWrap:'balance' }}>
    ...
    return wrap(
      <>
        ...
        <div style={{ position:'absolute', left: 56, right: 56, top: 400, textAlign:'center' }}>
          <h2 style={{ fontFamily: fonts.display, fontWeight: 400, fontSize: 112, lineHeight: 1.04, fontStyle:'italic', margin: 0, letterSpacing: '-0.025em', textWrap:'balance' }}>
            {String(slide.title || '').split(/\s+/).filter(Boolean).map((w, i, arr) => (
              <span key={i} style={{ color: i === Math.floor(arr.length / 2) ? pal.accent : pal.ink }}>{w}{' '}</span>
            ))}
          </h2>
