...
function SignalTemplate({ slide, idx, total, handle, year, pal, fonts, mark }) {
  ...
  if (slide.kind === 'cover') {
    // Split the title into words and tint the 2nd word accent, keeping the
    // rhythm of the original hardcoded design. textWrap: balance handles
    // line breaks naturally.
    const words = String(slide.title || '').split(/\s+/).filter(Boolean);
    return wrap(
      <>
        ...
        <div style={{ position: 'absolute', left: 40, right: 40, top: 300 }}>
          <div style={{
            fontFamily: fonts.display, fontSize: 168, lineHeight: 0.92,
            letterSpacing: '-0.035em', textTransform: 'uppercase', textWrap: 'balance',
          }}>
            {words.map((w, i) => (
  ...
  if (slide.kind === 'numbered') {
    return wrap(
      <>
        ...
        <div style={{ position: 'absolute', left: 40, right: 40, top: 660 }}>
          <h2 style={{
            fontFamily: fonts.display, fontSize: 96, lineHeight: 0.96, letterSpacing: '-0.03em',
            textTransform: 'uppercase', margin: 0, textWrap: 'balance',
          }}>{slide.title}</h2>
        </div>
        <div style={{ position: 'absolute', left: 56, right: 56, bottom: 160 }}>
          <div style={{ fontSize: 24, lineHeight: 1.4, maxWidth: 860, paddingTop: 20, borderTop: `2px solid ${pal.ink}` }}>
            {slide.body}
          </div>
        </div>
      </>
  ...
  if (slide.kind === 'stat') {
    return wrap(
      <>
        <div style={{
          position: 'absolute', left: 40, right: 40, top: 220,
          textAlign: 'center', fontFamily: fonts.display,
          fontSize: 460, lineHeight: 0.86, color: pal.ink,
          ...
          {String(slide.big || '').includes(':') ? (
            ...
            <span style={{ color: pal.ink }}>{slide.big}</span>
          )}
        </div>
        <div style={{
