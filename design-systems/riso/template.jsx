...
function RisoTemplate({ slide, idx, total, handle, year, pal, fonts, mark }) {
  ...
  if (slide.kind === 'cover') {
    return wrap(
      <>
        ...
        <div style={{ position:'absolute', left: 56, right: 56, top: 320, fontFamily: fonts.display, fontWeight: 800, fontSize: 132, lineHeight: 0.96, letterSpacing: '-0.035em', textTransform: 'uppercase', textWrap: 'balance' }}>
          {String(slide.title || '').split(/\s+/).filter(Boolean).map((w, i, arr) => (
            <span key={i}>
              {misprint(w, 132, i % 2 === 0 ? pal.ink : pal.accent, i % 2 === 0 ? pal.accent : pal.ink)}
              {' '}
            </span>
          ))}
        </div>
  ...
  if (slide.kind === 'stat') {
    return wrap(
      <>
        <div style={{ position:'absolute', left: 40, right: 40, top: 320, textAlign:'center' }}>
          <div style={{ fontFamily: fonts.display, fontWeight: 800, fontSize: 440, lineHeight: 0.88, letterSpacing: '-0.05em' }}>
            {misprint(slide.big, 440, pal.ink, pal.accent)}
          </div>
        </div>
  ...
  if (slide.kind === 'cta') {
    return wrap(
      <>
        <div style={{ position:'absolute', left: 56, right: 56, top: 320, textAlign:'center', fontFamily: fonts.display, fontWeight: 800, fontSize: 136, lineHeight: 0.95, textTransform: 'uppercase', letterSpacing: '-0.035em', textWrap: 'balance' }}>
          {String(slide.title || '').split(/\s+/).filter(Boolean).map((w, i, arr) => (
            <span key={i}>
              {misprint(w, 136, i % 2 === 0 ? pal.ink : pal.accent, i % 2 === 0 ? pal.accent : pal.ink)}
              {' '}
            </span>
          ))}
        </div>
