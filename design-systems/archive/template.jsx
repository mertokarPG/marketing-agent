    ...
    return wrap(
      <>
        <div style={{ position:'absolute', left: 56, right: 56, top: 180, textAlign:'center' }}>
          ...
          <h1 style={{ fontFamily: fonts.display, fontWeight: 900, fontSize: 128, lineHeight: 0.98, letterSpacing: '-0.02em', margin: 0, textWrap: 'balance', fontStyle: 'italic' }}>
            {slide.title}
          </h1>
          {slide.subtitle ? (
            <div style={{ fontFamily: fonts.display, fontWeight: 400, fontSize: 48, fontStyle: 'italic', marginTop: 24, color: pal.accent, textWrap: 'balance' }}>
              {slide.subtitle}
            </div>
          ) : null}
        </div>
        <div style={{ position:'absolute', left: 56, right: 56, top: 900, borderTop: `1px solid ${pal.rule}`, paddingTop: 30, display:'flex', justifyContent:'space-between', fontFamily: fonts.mono, fontSize: 12, color: pal.muted, letterSpacing: '0.16em', textTransform: 'uppercase' }}>
          <span>— Masthead —</span>
          <span style={{ fontVariantNumeric: 'tabular-nums' }}>№ {String(idx + 1).padStart(2, '0')}</span>
          <span>{year}</span>
        </div>
      </>
    );
